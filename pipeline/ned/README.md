...
- **NED:** Normalize (`pipeline/pkg/ti/normalize`, AppSec YAML parse) → Enrich (NVD CWE/CPE via `pipeline/pkg/nvd/parse`) → Dedup (`commit` idempotency keys + JetStream `Nats-Msg-Id`)
