...
```bash
docker compose -f deploy/scrape/compose.yml -f deploy/pipeline/compose.yml -f deploy/knowledge/compose.yml up --build
```

Docker build context is the repository root; each Dockerfile copies only its layer tree (`scrape/`, `pipeline/`, or `knowledge/`).
