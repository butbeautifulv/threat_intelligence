// Package scrapev1 defines the raw scrape envelope published to NATS scrape.> subjects.
package scrapev1
...
func (e *Envelope) Validate() error {
	if e.SchemaVersion != CurrentSchemaVersion {
		return fmt.Errorf("scrapev1: unsupported schema_version %d (want %d)", e.SchemaVersion, CurrentSchemaVersion)
	}
	if strings.TrimSpace(e.Source) == "" {
		return errors.New("scrapev1: empty source")
	}
	if strings.TrimSpace(e.Kind) == "" {
		return errors.New("scrapev1: empty kind")
	}
	if strings.TrimSpace(e.ContentKey) == "" {
		return errors.New("scrapev1: empty content_key")
	}
	if strings.TrimSpace(e.ScrapedAt) == "" {
		return errors.New("scrapev1: empty scraped_at")
	}
	if len(e.Payload) == 0 || string(e.Payload) == "null" {
		return errors.New("scrapev1: empty payload")
	}
	return nil
}
