...
func Run(ctx context.Context, opts RunOptions) error {
	...
	jsPub, err := scrapepub.ConnectJetStreamAndStream(natsURL)
	if err != nil {
	...
	for _, src := range sources {
		...
		if !ok {
			return fmt.Errorf("scrape: no scrapev1 source constant for %q", name)
		}
		subj := subjectForSource(name)
		publishers[name] = scrapepub.NewDomainPublisher(jsPub, srcConst, subj)
	}
