...
import (
	...
	"github.com/butbeautifulv/threat_intelligence/scrape/sources/sbom/internal/cvesource"
	"github.com/butbeautifulv/threat_intelligence/scrape/sources/sbom/internal/feeds/ghsa"
)
...
func (r *Runner) runOSV(ctx context.Context) error {
	...
	for i, cve := range cves {
		...
		pl := scrapev1.SBOMOSVRaw{CVE: cve, OSVID: id, RawJSON: string(raw)}
		if err := r.pub.Publish(ctx, scrapev1.KindSBOMOSVJSON, "sbom:osv:"+cve, pl); err != nil {
...
func (r *Runner) runGHSA(ctx context.Context) error {
	...
	for i, p := range paths {
		...
		pl := scrapev1.SBOMGHSARaw{Path: p, Doc: doc}
		if err := r.pub.Publish(ctx, scrapev1.KindSBOMGHSAPath, "sbom:ghsa:"+p, pl); err != nil {
