...
import (
	...
	"github.com/butbeautifulv/threat_intelligence/scrape/sources/vuln/internal/proxypool"
...
func (u *ScraperUsecase) parseNVD(data []byte) ([]domain.Vulnerability, int, error) {
	parsed, total, err := nvdparse.ParsePage(data)
	if err != nil {
		return nil, 0, err
	}
	out := make([]domain.Vulnerability, 0, len(parsed))
	for _, p := range parsed {
		out = append(out, nvdToDomain(p))
	}
	return out, total, nil
}

func nvdToDomain(p nvdparse.Vulnerability) domain.Vulnerability {
	...
	if len(p.CPEs) > 0 {
		v.CPEs = make([]domain.CPE, len(p.CPEs))
...
func (u *ScraperUsecase) ScrapeNVD(ctx context.Context) error {
	...
	for total < 0 || start < total {
		...
		count += len(vulns)
		u.logger.Info("nvd page published", slog.Int("startIndex", start), slog.Int("pageCount", len(vulns)), slog.Int("totalResults", total))
		if len(vulns) == 0 {
			break
		}
		pages++
