...
import (
	...
	"github.com/butbeautifulv/threat_intelligence/scrape/sources/coderules/internal/feeds/cwe"
)
...
func (b *cweScrapeBridge) UpsertCWECatalog(ctx context.Context, cweID, name, description, status string) error {
	pl := scrapev1.CoderulesCWERaw{ID: cweID, Name: name, Description: description, Status: status}
	return b.pub.Publish(b.ctx, scrapev1.KindCoderulesCWERaw, "coderules:cwe:"+cweID, pl)
}
...
func (r *Runner) runSemgrep(ctx context.Context) error {
	...
	for len(q) > 0 && n < r.opt.MaxSemgrep {
		...
		for _, it := range items {
			...
			pl := scrapev1.CoderulesSemgrepRaw{Path: it.Path, RawYAML: string(raw)}
			if err := r.pub.Publish(ctx, scrapev1.KindCoderulesSemgrepRaw, "coderules:semgrep:"+it.Path, pl); err != nil {
				return err
			}
			n++
		}
...
func (r *Runner) runCodeQL(ctx context.Context) error {
	...
	for _, it := range items {
		...
		pl := scrapev1.CoderulesCodeQLRaw{Path: it.Path, Body: string(raw)}
		if err := r.pub.Publish(ctx, scrapev1.KindCoderulesCodeQLRaw, "coderules:codeql:"+it.Path, pl); err != nil {
			return err
		}
		n++
	}
