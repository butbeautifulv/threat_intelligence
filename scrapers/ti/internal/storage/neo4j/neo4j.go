...
func (s *Store) LinkCampaignIOC(ctx context.Context, campaignID string, i domain.IOC) error {
	params := map[string]any{
		"campaignID": campaignID,
		"iocID":      normalize.CanonicalID(i),
...
func (s *Store) LinkCampaignActor(ctx context.Context, campaignID string, actorName string) error {
...
func (s *Store) UpsertReport(ctx context.Context, r domain.Report) error {
	id := r.ID
	if id == "" {
		id = normalize.ReportStableID(r.Link)
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
...
func (s *Store) LinkReportMentionsIOC(ctx context.Context, reportID string, i domain.IOC) error {
	if reportID == "" {
		return nil
	}
	iid := normalize.CanonicalID(i)
	params := map[string]any{
		"reportID": reportID,
		"iocID":    iid,
		"ts":       time.Now().UTC().Format(time.RFC3339Nano),
