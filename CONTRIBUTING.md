...
**CI:** PRs and `main` pushes touching `pkg/`, `discovery/`, `pipeline/`, `knowledge/`, or `engage/` run [`.github/workflows/platform.yml`](.github/workflows/platform.yml) (`make test-platform-p7`); closed-loop Docker smoke runs on `main`/`master` push only.
