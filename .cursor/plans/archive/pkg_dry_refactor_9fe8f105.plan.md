...
- **Изоляция:** `scrape/`, `pipeline/`, `knowledge/` не импортируют друг друга; общение только через NATS + типы из `pkg/`.
...
- Каждый [`scrape/go.work`](scrape/go.work), [`pipeline/go.work`](pipeline/go.work), [`knowledge/go.work`](knowledge/go.work) добавляет нужные `../pkg/...` через `use (...)`.
...
| `docs-scrape-readme` | [`scrape/README.md`](scrape/README.md), [`pipeline/README.md`](pipeline/README.md), [`knowledge/README.md`](knowledge/README.md) |
...
- `pipeline` не импортирует ничего из `scrape/` или `knowledge/`.
