...
Текущая версия: [`versions.env`](versions.env) → `GRAPH_PACK_VERSION=v0.4.0`. Ingest-код на `main` не менялся с последнего релиза — **bump обязателен только для артефакта** (v0.4.1), не для `check-graph-version` (нет правок в `scrape/`, `pipeline/`, `knowledge/ingest/`, `pkg/`).
...
docker compose -f deploy/scrape/compose.yml -f deploy/pipeline/compose.yml -f deploy/knowledge/compose.yml ps
```
...
docker compose -f deploy/scrape/compose.yml -f deploy/pipeline/compose.yml -f deploy/knowledge/compose.yml \
  ps -a scrape_worker
