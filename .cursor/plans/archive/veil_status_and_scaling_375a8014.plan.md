...
|---------|--------|----------------|
| Три контекста (scrape → pipeline → graph) | **done** | [`scrape/`](scrape/), [`pipeline/`](pipeline/), [`graph/`](graph/) |
...
| Три слоя `scrape/`, `pipeline/`, `graph/` | Есть, сборка через layer `go.work` | **pass** (не single `go.mod`/layer — допустимо по плану) |
