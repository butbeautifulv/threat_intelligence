...
|---------|--------|----------------|
| Три контекста (scrape → pipeline → graph) | **done** | [`discovery/`](discovery/), [`pipeline/`](pipeline/), [`knowledge/`](graph/) |
...
| Graph без legacy | **done** | `knowledge/sources/*`, `knowledge/storage/*`, `knowledge/workeringest/*` |
...
| Три слоя `discovery/`, `pipeline/`, `knowledge/` | Есть, сборка через layer `go.work` | **pass** (не single `go.mod`/layer — допустимо по плану) |
...
| `internal/domain/` обязателен | Есть только в **ti, vuln, lola** (scrape + graph) | **partial** — нет в `ds`, `sbom`, `coderules`, `nuclei` (scrape) и `knowledge/sources/ds` |
| Документация без legacy-путей | [`docs/architecture/threatintel-runtime.md`](docs/architecture/threatintel-runtime.md), [`docs/architecture/ontology-appsec.md`](docs/architecture/ontology-appsec.md) ссылаются на `ingest/`, `scrapers/`, `pkg/` | **fail** |
| Масштабирование воркеров в deploy | Нет `replicas` / scale wrapper | **fail** (запрос пользователя) |

Прочие артефакты: [`knowledge/go.mod.ingest-graph.bak`](knowledge/go.mod.ingest-graph.bak), устаревшие комментарии `ingest/scrape/factory` в `scrapesource/source.go`, [`pipeline/pipeline_worker/README.md`](pipeline/pipeline_worker/README.md).
...
Файлы: [`deploy/pipeline/compose.yml`](deploy/pipeline/compose.yml), [`deploy/knowledge/compose.yml`](deploy/knowledge/compose.yml), [`deploy/knowledge/compose.full.yml`](deploy/knowledge/compose.full.yml), новый [`scripts/compose-up-full.sh`](scripts/compose-up-full.sh), [`deploy/README.md`](deploy/README.md).
...
| Обновить runtime/deploy docs на новые пути | [`docs/architecture/threatintel-runtime.md`](docs/architecture/threatintel-runtime.md), [`docs/architecture/ontology-appsec.md`](docs/architecture/ontology-appsec.md), [`docs/deploy.md`](docs/deploy.md) |
| Минимальные `internal/domain/` для ds/sbom/coderules/nuclei (типы сущностей + пустые пакеты при необходимости) | `scrape/sources/*/internal/domain/`, `knowledge/sources/ds/internal/domain/` |
| Удалить `knowledge/go.mod.ingest-graph.bak`, поправить stale comments/README | misc |
