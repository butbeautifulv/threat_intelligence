...
|---------|--------|----------------|
| Три контекста (scrape → pipeline → graph) | **done** | [`scrape/`](scrape/), [`pipeline/`](pipeline/), [`knowledge/`](knowledge/) |
| Два NATS-hop (`scrape.>` → `ingest.>`) | **done** | `pipeline_worker` PullSubscribe SCRAPE; `ingest_worker` PullSubscribe INGEST |
| Factory + 7 sources в `scrape_worker` | **done** | [`deploy/scrape/compose.yml`](deploy/scrape/compose.yml) `SCRAPE_SOURCES=ds,vuln,lola,ti,sbom,coderules,nuclei` |
...
| Три слоя `scrape/`, `pipeline/`, `knowledge/` | Есть, сборка через layer `go.work` | **pass** (не single `go.mod`/layer — допустимо по плану) |
