...
Файл: [`scrape/harvest/internal/sources/vuln/internal/usecase/scrape.go`](scrape/harvest/internal/sources/vuln/internal/usecase/scrape.go)
...
- Удалить `pkg/nvd/` из корневого [`pkg/go.mod`](pkg/go.mod); убрать из [`scrape/go.work`](scrape/go.work) / [`scrape/harvest/go.mod`](scrape/harvest/go.mod) любые `pkg` imports связанные с nvd (harvest после 1.1 не импортирует `pkg` для vuln NVD).
