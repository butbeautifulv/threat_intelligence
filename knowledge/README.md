...
```bash
make test-knowledge
make test-knowledge-serve    # knowledge/serve only, -race
```

```bash
cd knowledge/ingest && go build -o bin/ingest_worker ./cmd/ingest_worker
cd knowledge/serve && go build -o bin/api ./cmd/api && go build -o bin/mcp ./cmd/mcp
```
